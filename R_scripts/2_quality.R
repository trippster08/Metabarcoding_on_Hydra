...
quality_plots_F <- list()
max_x_values <- list()
quality_plots_F_better <- list()

for (gene in genes) {
  forward_reads <- actual_trimmed_reads[[gene]]$F
  sample_names <- sample_names_trimmed[[gene]]
  # Make original plots
  quality_plots_F[[gene]] <- plotQualityProfile(
    forward_reads[1:length(sample_names)],
    aggregate = TRUE
  )
  # Extract x axis range and maximum x values
  plot_build <- ggplot_build(quality_plots_F[[gene]])
  x_axis_range <- plot_build$layout$panel_params[[1]]$x.range
  max_x <- x_axis_range[2]
  max_x_values[[gene]] <- max_x

  # Enhance the plot
  quality_plots_F_better[[gene]] <- quality_plots_F[[gene]] +
    scale_x_continuous(
      limits = c(0, max_x),
  ...
  ggsave(
    filename = file.path(
      path_to_results,
      paste0(project_name, "_", gene, "_qualplotF_", gene, ".pdf")
    ),
...
# Do this for the reverse (R2) reads

quality_plots_R <- list()
max_x_values <- list()
quality_plots_R_better <- list()

for (gene in genes) {
  forward_reads <- actual_trimmed_reads[[gene]]$R
  sample_names <- sample_names_trimmed[[gene]]
  # Make original plots
  quality_plots_R[[gene]] <- plotQualityProfile(
    forward_reads[1:length(sample_names)],
    aggregate = TRUE
  )
  # Extract x axis range and maximum x values
  plot_build <- ggplot_build(quality_plots_R[[gene]])
  x_axis_range <- plot_build$layout$panel_params[[1]]$x.range
  max_x <- x_axis_range[2]
  max_x_values[[gene]] <- max_x

  # Enhance the plot
  quality_plots_R_better[[gene]] <- quality_plots_R[[gene]] +
    scale_x_continuous(
      limits = c(0, max_x),
      breaks = seq(0, max_x, 10)
    ) +
    ...
    annotate("text", x = 30, y = 2, label = paste0(gene, " R2"), size = 6)

  # Save the plot as a PDF
  ggsave(
    filename = file.path(
      path_to_results,
      paste0(project_name, "_", gene, "_qualplotR_", gene, ".pdf")
    ),
    plot = quality_plots_R_better[[gene]],
    width = 9,
    height = 9
  )
}
